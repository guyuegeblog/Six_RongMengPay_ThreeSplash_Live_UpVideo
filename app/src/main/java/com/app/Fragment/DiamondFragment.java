...
import com.app.Adapter.Diamond_MultiAdapter;
...
public class DiamondFragment extends LazyFragment implements BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {

    @Bind(R.id.progressbar)
    RelativeLayout progressbar;
    @Bind(R.id.diamond_recyclerview)
    RecyclerView diamondRecyclerview;
    @Bind(R.id.diamond_swipe_fresh)
    SwipeRefreshLayout diamondSwipeFresh;
    private MainActivity mContext;
    private View rootView;
    private List<Multi> jarpanse_Multi = new ArrayList<>();
    private Diamond_MultiAdapter diamond_multiAdapter;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case GO_LOOK_REQUEST_FAILED:
                    ...
                    diamondSwipeFresh.setRefreshing(false);
                    break;
    ...
    protected void onCreateViewLazy(Bundle savedInstanceState) {
        ...
        rootView = LayoutInflater.from(mContext).inflate(R.layout.fragment_diamond, null);
    ...
    private void initView() {
        ...
        diamondSwipeFresh.setOnRefreshListener(this);
        diamondSwipeFresh.setRefreshing(false);
        diamondRecyclerview.addItemDecoration(new Level_MarginDecoration(mContext));
        diamondRecyclerview.setHasFixedSize(true);
        diamondRecyclerview.setLayoutManager(new GridLayoutManager(mContext, 2));
        diamond_multiAdapter = new Diamond_MultiAdapter(jarpanse_Multi, mContext);
        diamond_multiAdapter.setSpanSizeLookup(new BaseQuickAdapter.SpanSizeLookup() {
            @Override
...
//        }
    ...
    private void adapterData() {
        diamond_multiAdapter.notifyDataSetChanged();
        mContext.getProgressBar().dismiss();
    }
    ...
    public void onRefresh() {
        diamond_multiAdapter.setEnableLoadMore(true);
        isFresh = true;
        onLoadMore = false;
        page = 0;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    ...
    public void onLoadMoreRequested() {
        diamondSwipeFresh.setEnabled(false);
        isFresh = false;
        onLoadMore = true;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    }
    ...
    private void onLoadMore() {
        diamondRecyclerview.postDelayed(new Runnable() {
            @Override
            public void run() {
//              Log.i("MainLog", "three_mCurrentCounter==" + three_mCurrentCounter + " three_TOTAL_COUNTER" + three_TOTAL_COUNTER);
                if (mCurrentCounter >= mTOTAL_COUNTER) {
                    diamond_multiAdapter.loadMoreEnd();
//                    alertVipPay();
                } else {
                    if (!isErr) {
                        page++;
                        diamond_multiAdapter.addData(initJarpanseData(Integer.parseInt(String.valueOf(page)), aesLookInfoList));
                        diamond_multiAdapter.loadMoreComplete();
                    } else {
                        Toast.makeText(mContext, "加载失败，请重新再试!", Toast.LENGTH_LONG).show();
                        diamond_multiAdapter.loadMoreFail();
                    }
                }
                diamondSwipeFresh.setEnabled(true);
            }
        }, delayMillis);
    }
...
//              three_Id = 0;
                ...
                isErr = false;
                diamond_multiAdapter.setEnableLoadMore(true);
                diamond_multiAdapter.setNewData(jarpanse_Multi);
            }
        }, delayMillis);
    }
