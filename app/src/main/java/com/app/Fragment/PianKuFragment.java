...
import com.app.Adapter.PianKu_MultiAdapter;
...
public class PianKuFragment extends LazyFragment implements BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {

    @Bind(R.id.progressbar)
    RelativeLayout progressbar;
    @Bind(R.id.pianku_recyclerview)
    RecyclerView piankuRecyclerview;
    @Bind(R.id.pianku_swipe_fresh)
    SwipeRefreshLayout piankuSwipeFresh;
    private MainActivity mContext;
    private View rootView;
    private List<Multi> jarpanse_Multi = new ArrayList<>();
    private PianKu_MultiAdapter pianKu_multiAdapter;
    private boolean isFresh = false;
    private boolean onLoadMore = false;
    private int delayMillis = 10;
    private int page = 0;
    private int mCurrentCounter = 0;
    private int mTOTAL_COUNTER = 8;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case GO_LOOK_REQUEST_FAILED:
                    ...
                    piankuSwipeFresh.setRefreshing(false);
                    break;
    ...
    protected void onCreateViewLazy(Bundle savedInstanceState) {
        ...
        rootView = LayoutInflater.from(mContext).inflate(R.layout.fragment_pianku, null);
    ...
    private void initView() {
        ...
        piankuSwipeFresh.setOnRefreshListener(this);
        piankuSwipeFresh.setRefreshing(false);
//        piankuRecyclerview.addItemDecoration(new Level_MarginDecoration(mContext));
        piankuRecyclerview.setHasFixedSize(true);
        piankuRecyclerview.setLayoutManager(new GridLayoutManager(mContext, 1));
        pianKu_multiAdapter = new PianKu_MultiAdapter(jarpanse_Multi, mContext);
        pianKu_multiAdapter.setSpanSizeLookup(new BaseQuickAdapter.SpanSizeLookup() {
            @Override
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                try {
                    ...
                    for (int i = 1; i <= 8; i++) {
                        urlString.add("http://www.yimeiyichu.com/lx/pd" + i + ".jpg");
                    }
...
//                        comprehenSiveInfo.setRandomstr(AesTool.decrypt(jsonObjectList.get(i).getString("randomstr")));
                        comprehenSiveInfo.setPic(urlString.get(i));
...
//        }
    ...
    private void adapterData() {
        pianKu_multiAdapter.notifyDataSetChanged();
        mContext.getProgressBar().dismiss();
    }
    ...
    public void onRefresh() {
        pianKu_multiAdapter.setEnableLoadMore(true);
        isFresh = true;
        onLoadMore = false;
        page = 0;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    ...
    public void onLoadMoreRequested() {
        piankuSwipeFresh.setEnabled(false);
        isFresh = false;
        onLoadMore = true;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    }
    ...
    private void onLoadMore() {
        piankuRecyclerview.postDelayed(new Runnable() {
            @Override
            public void run() {
//              Log.i("MainLog", "three_mCurrentCounter==" + three_mCurrentCounter + " three_TOTAL_COUNTER" + three_TOTAL_COUNTER);
                if (mCurrentCounter >= mTOTAL_COUNTER) {
                    pianKu_multiAdapter.loadMoreEnd();
//                    alertVipPay();
                } else {
                    if (!isErr) {
                        page++;
                        pianKu_multiAdapter.addData(initJarpanseData(Integer.parseInt(String.valueOf(page)), aesLookInfoList));
                        pianKu_multiAdapter.loadMoreComplete();
                    } else {
                        Toast.makeText(mContext, "加载失败，请重新再试!", Toast.LENGTH_LONG).show();
                        pianKu_multiAdapter.loadMoreFail();
                    }
                }
                piankuSwipeFresh.setEnabled(true);
            }
        }, delayMillis);
    }
...
//              three_Id = 0;
                ...
                isErr = false;
                pianKu_multiAdapter.setEnableLoadMore(true);
                pianKu_multiAdapter.setNewData(jarpanse_Multi);
            }
        }, delayMillis);
    }
