...
import com.app.Adapter.PlatNum_MultiAdapter;
...
public class PlatNumFragment extends LazyFragment implements BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {

    @Bind(R.id.progressbar)
    RelativeLayout progressbar;
    @Bind(R.id.platnum_recyclerview)
    RecyclerView platnumRecyclerview;
    @Bind(R.id.platnum_swipe_fresh)
    SwipeRefreshLayout platnumSwipeFresh;
    private MainActivity mContext;
    private View rootView;
    private List<Multi> jarpanse_Multi = new ArrayList<>();
    private PlatNum_MultiAdapter platNum_multiAdapter;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case GO_LOOK_REQUEST_FAILED:
                    ...
                    platnumSwipeFresh.setRefreshing(false);
                    break;
    ...
    protected void onCreateViewLazy(Bundle savedInstanceState) {
        ...
        rootView = LayoutInflater.from(mContext).inflate(R.layout.fragment_platnum, null);
    ...
    private void initView() {
        ...
        platnumSwipeFresh.setOnRefreshListener(this);
        platnumSwipeFresh.setRefreshing(false);
        platnumRecyclerview.addItemDecoration(new Level_MarginDecoration(mContext));
        platnumRecyclerview.setHasFixedSize(true);
        platnumRecyclerview.setLayoutManager(new GridLayoutManager(mContext, 2));
        platNum_multiAdapter = new PlatNum_MultiAdapter(jarpanse_Multi, mContext);
        platNum_multiAdapter.setSpanSizeLookup(new BaseQuickAdapter.SpanSizeLookup() {
            @Override
...
//        }
    ...
    private void adapterData() {
        platNum_multiAdapter.notifyDataSetChanged();
        mContext.getProgressBar().dismiss();
    }
    ...
    public void onRefresh() {
        platNum_multiAdapter.setEnableLoadMore(true);
        isFresh = true;
        onLoadMore = false;
        page = 0;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    ...
    public void onLoadMoreRequested() {
        platnumSwipeFresh.setEnabled(false);
        isFresh = false;
        onLoadMore = true;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    }
    ...
    private void onLoadMore() {
        platnumRecyclerview.postDelayed(new Runnable() {
            @Override
            public void run() {
//              Log.i("MainLog", "three_mCurrentCounter==" + three_mCurrentCounter + " three_TOTAL_COUNTER" + three_TOTAL_COUNTER);
                if (mCurrentCounter >= mTOTAL_COUNTER) {
                    platNum_multiAdapter.loadMoreEnd();
//                    alertVipPay();
                } else {
                    if (!isErr) {
                        page++;
                        platNum_multiAdapter.addData(initJarpanseData(Integer.parseInt(String.valueOf(page)), aesLookInfoList));
                        platNum_multiAdapter.loadMoreComplete();
                    } else {
                        Toast.makeText(mContext, "加载失败，请重新再试!", Toast.LENGTH_LONG).show();
                        platNum_multiAdapter.loadMoreFail();
                    }
                }
                platnumSwipeFresh.setEnabled(true);
            }
        }, delayMillis);
    }
...
//              three_Id = 0;
                ...
                isErr = false;
                platNum_multiAdapter.setEnableLoadMore(true);
                platNum_multiAdapter.setNewData(jarpanse_Multi);
            }
        }, delayMillis);
    }
