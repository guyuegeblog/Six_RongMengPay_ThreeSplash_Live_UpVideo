...
import com.app.Adapter.RedDiamond_MultiAdapter;
...
public class RedDiamondFragment extends LazyFragment implements BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {

    @Bind(R.id.progressbar)
    RelativeLayout progressbar;
    @Bind(R.id.rediamond_recyclerview)
    RecyclerView rediamondRecyclerview;
    @Bind(R.id.rediamond_swipe_fresh)
    SwipeRefreshLayout rediamondSwipeFresh;
    private MainActivity mContext;
    private View rootView;
    private List<Multi> jarpanse_Multi = new ArrayList<>();
    private RedDiamond_MultiAdapter redDiamond_multiAdapter;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case GO_LOOK_REQUEST_FAILED:
                    ...
                    rediamondSwipeFresh.setRefreshing(false);
                    break;
    ...
    protected void onCreateViewLazy(Bundle savedInstanceState) {
        ...
        rootView = LayoutInflater.from(mContext).inflate(R.layout.fragment_red_diamond, null);
    ...
    private void initView() {
        ...
        rediamondSwipeFresh.setOnRefreshListener(this);
        rediamondSwipeFresh.setRefreshing(false);
        rediamondRecyclerview.addItemDecoration(new Level_MarginDecoration(mContext));
        rediamondRecyclerview.setHasFixedSize(true);
        rediamondRecyclerview.setLayoutManager(new GridLayoutManager(mContext, 2));
        redDiamond_multiAdapter = new RedDiamond_MultiAdapter(jarpanse_Multi, mContext);
        redDiamond_multiAdapter.setSpanSizeLookup(new BaseQuickAdapter.SpanSizeLookup() {
            @Override
...
//        }
    ...
    private void adapterData() {
        redDiamond_multiAdapter.notifyDataSetChanged();
        mContext.getProgressBar().dismiss();
    }
    ...
    public void onRefresh() {
        redDiamond_multiAdapter.setEnableLoadMore(true);
        isFresh = true;
        onLoadMore = false;
        page = 0;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    ...
    public void onLoadMoreRequested() {
        rediamondSwipeFresh.setEnabled(false);
        isFresh = false;
        onLoadMore = true;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    }
    ...
    private void onLoadMore() {
        rediamondRecyclerview.postDelayed(new Runnable() {
            @Override
            public void run() {
//              Log.i("MainLog", "three_mCurrentCounter==" + three_mCurrentCounter + " three_TOTAL_COUNTER" + three_TOTAL_COUNTER);
                if (mCurrentCounter >= mTOTAL_COUNTER) {
                    redDiamond_multiAdapter.loadMoreEnd();
//                    alertVipPay();
                } else {
                    if (!isErr) {
                        page++;
                        redDiamond_multiAdapter.addData(initJarpanseData(Integer.parseInt(String.valueOf(page)), aesLookInfoList));
                        redDiamond_multiAdapter.loadMoreComplete();
                    } else {
                        Toast.makeText(mContext, "加载失败，请重新再试!", Toast.LENGTH_LONG).show();
                        redDiamond_multiAdapter.loadMoreFail();
                    }
                }
                rediamondSwipeFresh.setEnabled(true);
            }
        }, delayMillis);
    }
...
//              three_Id = 0;
                ...
                isErr = false;
                redDiamond_multiAdapter.setEnableLoadMore(true);
                redDiamond_multiAdapter.setNewData(jarpanse_Multi);
            }
        }, delayMillis);
    }
