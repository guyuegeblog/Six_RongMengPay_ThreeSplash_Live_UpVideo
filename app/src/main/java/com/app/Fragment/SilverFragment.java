...
import com.app.Adapter.Silver_MultiAdapter;
...
public class SilverFragment extends LazyFragment implements BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {

    @Bind(R.id.progressbar)
    RelativeLayout progressbar;
    @Bind(R.id.silver_recyclerview)
    RecyclerView silverRecyclerview;
    @Bind(R.id.silver_swipe_fresh)
    SwipeRefreshLayout silverSwipeFresh;
    private MainActivity mContext;
    private View rootView;
    private List<Multi> jarpanse_Multi = new ArrayList<>();
    private Silver_MultiAdapter silver_multiAdapter;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case GO_LOOK_REQUEST_FAILED:
                    ...
                    silverSwipeFresh.setRefreshing(false);
                    break;
    ...
    protected void onCreateViewLazy(Bundle savedInstanceState) {
        ...
        rootView = LayoutInflater.from(mContext).inflate(R.layout.fragment_silver, null);
    ...
    private void initView() {
        ...
        silverSwipeFresh.setOnRefreshListener(this);
        silverSwipeFresh.setRefreshing(false);
        silverRecyclerview.addItemDecoration(new Level_MarginDecoration(mContext));
        silverRecyclerview.setHasFixedSize(true);
        silverRecyclerview.setLayoutManager(new GridLayoutManager(mContext, 2));
        silver_multiAdapter = new Silver_MultiAdapter(jarpanse_Multi, mContext);
        silver_multiAdapter.setSpanSizeLookup(new BaseQuickAdapter.SpanSizeLookup() {
            @Override
...
//        }
    ...
    private void adapterData() {
        silver_multiAdapter.notifyDataSetChanged();
        mContext.getProgressBar().dismiss();
    }
    ...
    public void onRefresh() {
        silver_multiAdapter.setEnableLoadMore(true);
        isFresh = true;
        onLoadMore = false;
        page = 0;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    ...
    public void onLoadMoreRequested() {
        silverSwipeFresh.setEnabled(false);
        isFresh = false;
        onLoadMore = true;
        getNetLookData(Multi.COMPREHENSIVE_DUANSHIPIN / 1000 + "");
    }
    ...
    private void onLoadMore() {
        silverRecyclerview.postDelayed(new Runnable() {
            @Override
            public void run() {
//              Log.i("MainLog", "three_mCurrentCounter==" + three_mCurrentCounter + " three_TOTAL_COUNTER" + three_TOTAL_COUNTER);
                if (mCurrentCounter >= mTOTAL_COUNTER) {
                    silver_multiAdapter.loadMoreEnd();
//                    alertVipPay();
                } else {
                    if (!isErr) {
                        page++;
                        silver_multiAdapter.addData(initJarpanseData(Integer.parseInt(String.valueOf(page)), aesLookInfoList));
                        silver_multiAdapter.loadMoreComplete();
                    } else {
                        Toast.makeText(mContext, "加载失败，请重新再试!", Toast.LENGTH_LONG).show();
                        silver_multiAdapter.loadMoreFail();
                    }
                }
                silverSwipeFresh.setEnabled(true);
            }
        }, delayMillis);
    }
...
//              three_Id = 0;
                ...
                isErr = false;
                silver_multiAdapter.setEnableLoadMore(true);
                silver_multiAdapter.setNewData(jarpanse_Multi);
            }
        }, delayMillis);
    }
